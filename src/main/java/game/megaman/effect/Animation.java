...
public class Animation {
	...
	public void setIgnoreFrame(int id) {
		if(id>=0&&id<ignoreFrames.size())     ignoreFrames.set(id,true);
	}
	public void unIgnoreFrame(int id) {
		if(id>=0&&id<ignoreFrames.size())     ignoreFrames.set(id,false);
	}
	public void reset() {
		currentFrame=0;
		beginTime=0;
	...
	public void draw(int x, int y, Graphics2D g2) {
		...
		if(drawRectFrame)    g2.drawRect(x - image.getWidth()/2, x - image.getWidth()/2, image.getWidth(), image.getHeight());
	}
}
